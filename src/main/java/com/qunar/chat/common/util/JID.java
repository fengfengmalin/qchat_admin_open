...
public class JID implements Comparable<JID>, Serializable {
     ...
     *
     * @param node The raw node value.
     * @return A String based JID node representation
     * @throws IllegalStateException    If the UTF-8 charset is not supported by the system. This
     *                                  exception wraps an UnsupportedEncodingException.
     * @throws IllegalArgumentException if <tt>node</tt> is not a valid JID node.
     */
     ...
     * @throws IllegalStateException    If the UTF-8 charset is not supported by the system. This
     *                                  exception wraps an UnsupportedEncodingException.
     * @throws IllegalArgumentException if <tt>domain</tt> is not a valid JID domain part.
     */
     ...
     * StringPrepping fails).
     *
     * @param resource The raw resource value.
     * @return A String based JID resource representation
     * @throws IllegalStateException    If the UTF-8 charset is not supported by the system. This
     *                                  exception wraps an UnsupportedEncodingException.
     * @throws IllegalArgumentException if <tt>resource</tt> is not a valid JID resource.
     */
...
//    public JID(String jid, boolean skipStringPrep, boolean skipResourcesOnly) {
//        this(getParts(jid), false, true);
//    }
    ...
    private JID(String[] parts, boolean skipStringPrep, boolean skipResourcesOnly) {
        this(parts[0], parts[1], parts[2], skipStringPrep, skipResourcesOnly);
    }
    ...
    public JID(String node, String domain, String resource, boolean skipStringprep) {
        this(node, domain, resource, skipStringprep, false);
    }

    public JID(String node, String domain, String resource, boolean skipStringprep, boolean skipresourcesOnly) {
        if (domain == null) {
     ...
     *
     * @return the full JID.
     * @throws IllegalStateException If no resource was provided in the constructor used to create
     *                               this instance.
     */
...
//                    + "JID representation is not available for: " + toString());
    ...
    public static JID parseAsJID(String jid) {
        return new JID(getParts(jid.trim()), false, true);
    }


    public static void main(String[] args) {
        Map<JID, String> packet = new HashMap<JID, String>();
        packet.put(JID.parseAsJID("dan.liu@qunar.com/asdfasdflkj"), "3");
        packet.put(JID.parseAsJID("dan.liu@qunar.com"), "1");
        packet.put(JID.parseAsJID("lffan.liu@qunar.com"), "2");
    }
}
