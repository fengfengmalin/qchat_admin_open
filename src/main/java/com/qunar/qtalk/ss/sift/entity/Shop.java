...
public class Shop {
    ...
    public static long parseString(String shopId) {
        if (StringUtils.isNotEmpty(shopId)) {
            if (shopId.startsWith("shop_")) {
                String tmp = shopId.replace("shop_", "");

                if (StringUtils.isNumeric(tmp))
                    return Long.parseLong(tmp);
                else {
                    JID tmpId = JID.parseAsJID(tmp);
                    if (tmpId != null) {
                        return Long.parseLong(tmpId.getNode());
                    }
                }
            } else if (StringUtils.isNumeric(shopId)) {
